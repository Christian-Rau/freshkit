...
( async () => {
  ...
  console.log( kleur.cyan( `` ) );
  console.log( kleur.yellow( `Installing dependencies with ${ packageManager }...` ) );
  await runCommand( packageManager === 'pnpm' ? 'pnpm install' : packageManager === 'npm' ? 'npm install' : 'yarn' );
  ...
  if ( setupTailwind ) {
    console.log( `` );
    console.log( ( ` ` ), kleur.green( 'Okay!' ), kleur.yellow( 'Setting up Tailwind CSS...' ) );
    await runCommand( `${ packageManager } install tailwindcss postcss autoprefixer` );
    ...
    fs.writeFileSync( 'src/routes/+page.svelte', ` <div class="flex h-screen justify-center items-center">
  ...
  console.log( `  ${ i++ }: ${ kleur.bold( kleur.cyan( 'git init && git add -A && git commit -m "Initial commit"' ) ) } (optional)` );
  console.log( `  ${ i++ }: ${ kleur.bold( kleur.cyan( 'npm run dev -- --open' ) ) }` );

  console.log( `\nTo close the dev server, hit ${ kleur.bold( kleur.cyan( 'Ctrl-C' ) ) }` );
  console.log( `\nFeedback? Visit me at ${ kleur.cyan( 'https://github.com/christian-rau/freshkit' ) }` );
...
async function runPrettier() {
  try {
    ...
    console.error( kleur.red( 'Prettier is not installed. Install it with `npm install --save-dev prettier` or `yarn add --dev prettier`.' ) );
    process.exit( 1 );
  }
